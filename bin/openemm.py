...
if tomcathome is None:
	...
	for fname in sorted ([_f for _f in os.listdir (home + os.path.sep+"external"+os.path.sep) if _f.startswith ('apache-tomcat-') and not _f.endswith('.zip')]):
		tomcathome = os.path.sep.join ([home,"external", fname])
	if tomcathome is None:
		error ('Tomcat not found')
	else:
		print("Found Tomcat:"+tomcathome)
	...
	if sys.argv[1] == 'setup':
		def findSQL (prefix):
			for fname in ['%s.sql' % prefix, '%s-%s.sql' % (prefix, curversion)]:
				...
				if os.path.isfile (path):
					return path
			#Added by Jojo, for set up openemm_cms database, no openemm_cms-2015_R2.sql found, use openemm_cms-2015.sql instead.
			
			for fname in ['%s.sql' % prefix, '%s-%s.sql' % (prefix, curversion2)]:
				path = 'USR_SHARE\\%s' % fname
				if os.path.isfile (path):
					return path

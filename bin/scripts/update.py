...
class Update: #{{{
    ...
    def mp_update(self, inst, q):
        if sys.version_info.major == 2 and sys.version_info.minor < 6:
            raise Exception("Not supported for Python 2.5, no multiprocessing available!")
        q.put (self.update (inst))
#}}}
class UpdateDetail: #{{{
    Ignore = 0
    Internal = 100
    ...
    Softbounces = (SoftbounceOther, SoftbounceReceiver, SoftbounceMailbox , 430, 500)
...
class UpdateBounce (Update): #{{{
    ...
    def __todetail (self, dsn, info, company):
        (ignore, detail, code, typ, remark) = (False, 0, 0, agn.UserStatus.BOUNCE, 'bounce')
        ...
        if not match is None:
            ...
            if detail == 0:
                if (code in (511, 571) and (stat.find ('user unknown') != -1 or not mailloop is None)) or code == 513:
                    ...
                    detail = UpdateDetail.Success
                    typ = 1
                elif code / 100 == 1:
                    detail = UpdateDetail.Ignore
                    typ = 1
                else:
    ...
    def updateEnd (self, inst):
        ...
        agn.log (agn.LV_INFO, 'udpBounce', 'Found %d hardbounces, %d softbounces, %d successes, %d blacklisted, %d ignored in %d lines' % (self.hbcount, self.sbcount, self.sucount, self.blcount, self.igcount, self.lineno))
        return True
    ...
    def updateLine (self, inst, line):
        ...
        company = 1
        (ignore, detail, dsnnr, bouncetype, bounceremark) = self.__todetail (dsn, info, company)
